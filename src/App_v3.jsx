...
// Constants & Utils
import { INITIAL_ARTICLES, INITIAL_USUARIS, UBICACIONS, DEFAULT_SCRIPT_URL, CHART_COLORS } from './constants/index_v3';
import { robustParseDate, formatCurrentDate, GENERATE_YEARS } from './utils/dateUtils';
import { useLongPress } from './hooks/useLongPress';

// Components
import AppHeader from './components/AppHeader';
import ProductGrid from './components/ProductGrid_v3'; // LOAD V3
import MateriaSecaGrid from './components/MateriaSecaGrid'; // NEW
import EntryDialog from './components/EntryDialog_v3';
...
function App() {
    ...
    const [materialStock, setMaterialStock] = useState({}); // NOU
    ...
    const fetchGlobalConfig = async () => {
        try {
            ...
            if (data.materialStock) setMaterialStock(data.materialStock); // NOU
        } catch (e) {
    ...
    const handleSaveEntry = () => {
        ...
        const bottlesPerBox = selectedArticle === 'CERVESA PETITA' ? 16 : (isMateriaSeca ? 1 : 6);
        ...
        const entry = {
            ...
            image: photo,
            locationSource: 'App Mobil v3',
            sheet: targetSheet // Aquesta és la clau que rep el GAS
        };
        ...
        setSnackbar({
            open: true,
            message: `Guardat per enviar a: ${targetSheet}`,
            severity: isMateriaSeca ? 'info' : 'success'
        });
    };
                ...
                <Box sx={{ mt: 3, px: 2 }}>
                    {view === 'grid' ? (
                        <ProductGrid
                            ...
                            onOpenMateriaSeca={() => setView('materia_seca')}
                        ...
                        <MateriaSecaGrid
                            ...
                            onBack={() => setView('grid')}
                            darkMode={darkMode}
                            isSetupComplete={!!(usuari && ubicacio)}
                        />
                    ) : (
                        <StockDashboard
